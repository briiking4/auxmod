...
const FilterScores = async (songTitle, songArtist, chosenFilters) => {
  ...
  // console.log("profanity filter: " + profanityFilter?.options.whitelist);
    ...
    const getLyrics = async (songTitle, songArtist) => {
      try {
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/lyrics?artist=${encodeURIComponent(songArtist)}&title=${encodeURIComponent(songTitle)}`, {
          method: 'GET',
          headers: {
